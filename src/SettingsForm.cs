...
namespace AstralPartyModManager
{
    ...
    public partial class SettingsForm : Form
    {
        ...
        {
            txtGamePath.Text = _configManager.GamePath;
            txtModPath.Text = _configManager.ModPath;
            chkBackup.Checked = _configManager.BackupEnabled;
            chkConflict.Checked = _configManager.AutoDetectConflicts;
        }

        private void btnSave_Click(object sender, EventArgs e)
        {
            _configManager.GamePath = txtGamePath.Text;
            _configManager.ModPath = txtModPath.Text;
            _configManager.BackupEnabled = chkBackup.Checked;
            _configManager.AutoDetectConflicts = chkConflict.Checked;

            if (_configManager.ValidatePaths(out string errorMessage))
            {
        ...
        private void btnBrowseMod_Click(object sender, EventArgs e)
        {
            using (var dialog = new FolderBrowserDialog())
            {
                ...
                dialog.SelectedPath = txtModPath.Text;
        ...
        private void btnVerify_Click(object sender, EventArgs e)
        {
            ...
            if (_configManager.ValidatePaths(out string errorMessage))
            {
                MessageBox.Show("路径验证通过！", "成功", 
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show($"路径验证失败：{errorMessage}", "错误", 
